      ...
      <form onSubmit={handleQuizSubmit}>
        {questions.map((question, index) => (
          <div key={index} className="mb-4">
            ...
            {/* After submission, display the correct answer */}
            {submitted && (
              <div className="mt-2">
                <p
                  className={
                    answers[index]?.toLowerCase() === question.correct.toLowerCase()
                      ? 'text-green-600'
                      : 'text-red-600'
                  }
                >
                  {answers[index] ? `Your answer: ${answers[index]}` : 'No answer given'}
                </p>
      ...
      {submitted && (
        <div className="mt-4">
          <h3 className="text-xl">
            Your Score: {questions.filter((_, idx) => answers[idx]?.toLowerCase() === questions[idx].correct?.toLowerCase()).length}/{questions.length}
          </h3>
        </div>
      )}
    </div>
